...
import gogAbi from "../artifacts/contracts/GOG.sol/GOG.json";
import {
  onLogout
...
let readGogContract, readProvider
...
export default function Mint() {
  ...
  const [whitelistMinted, setWhitelistMinted] = useState(false)
  ...
  const gogContract = useSelector((state) => {
    return state.user.gogContract;
  });

  const thresholds = [1000, 2000, 3000, 4000, 5000, 6000, 7070]
    ...
    (async () => {
      ...
      try {
        if (!readProvider) {
          ...
          readGogContract.totalMinted().then(tMinted => setTotalMinted(tMinted.toNumber())).catch(err => errorAlert('Error getting total supply:', err))
          readGogContract.saleState().then(newState => setSaleState(newState)).catch(err => errorAlert('Error getting sale state:', err))
          readGogContract.MAX_ENTRIES().then(maxSpl => setMaxSupply(maxSpl.toNumber())).catch(err => errorAlert('Error getting max supply:', err))
          readGogContract.LIMIT_PER_TRANSACTION().then(newLimit => setWalletLimit(newLimit.toNumber())).catch(err => errorAlert('Error getting limit per transaction:', err))
        }
        if (!!walletAddress) {
          readProvider.getBalance(walletAddress).then(blnc => setBalance(blnc.toString())).catch(err => errorAlert('Error getting balance:', err))
          readGogContract.minted(walletAddress).then(minted => setWhitelistMinted(minted)).catch(err => errorAlert('Error getting whitelistminted:', err))
          readGogContract.whitelisted(walletAddress).then(whl => setWhitelisted(whl)).catch(err => errorAlert('Error getting whitelisted:', err))
        }
        let newPrices = []
        for (let i = 0; i < 7; ++i) {
          newPrices = [...newPrices, (await readGogContract.PRICES(i)).toString()]
        }
        setPrices([...newPrices])
      } catch (err) {
  ...
  const getPrice = index => {
    ...
    const thresIndex = thresholds.findIndex(hold => hold >= index)
    if (thresIndex < 0) {
      return '0'
    }
    return prices[thresIndex]
  }

  useEffect(() => {
    let newPrice = '0'
    for (let i = 1; i <= mintAmount; ++i) {
      console.log('currentprice:', getPrice(i + totalMinted))
      newPrice = BigNumber.from(newPrice).add(BigNumber.from(getPrice(i + totalMinted))).toString()
    }
    setTotalPrice(newPrice)
  }, [mintAmount, prices.length])
  ...
  const mint = async () => {
    ...
    if (saleState === 1) {
      ...
      } else if (whitelistMinted) {
        return setStatus({
          success: false,
          message: 'Already minted whitelist nft'
        })
      }
    }
    if (mintAmount <= 0) {
    ...
    if (mintAmount > walletLimit) {
      return setStatus({
        success: false,
        message: 'Exceeds max limits per tx'
      })
    }
    ...
    setIsMinting(true)
    const gasEstimated = await gogContract.estimateGas.mint(mintAmount, {
      value: saleState === 1 ? '0' : totalPrice,
    });
    let tx
    if (useHigherGas) {
      const gas = Math.ceil(gasEstimated.toNumber() * 1.5);
      const gasNumber = BigNumber.from(gas);
      tx = await gogContract.mint(mintAmount, {
        value: saleState === 1 ? '0' : totalPrice,
        gasLimit: gasNumber,
      });
    } else {
      tx = await gogContract.mint(mintAmount, { value: saleState === 1 ? '0' : totalPrice })
    }
    try {
      await tx.wait()
    } catch (err) {
        ...
        <div className="flex flex-col items-center justify-center h-full w-full px-2 md:px-10">
          <div className="relative z-1 md:max-w-3xl w-full bg-gray-900/90 filter backdrop-blur-sm py-4 rounded-md px-2 md:px-10 flex flex-col items-center">
            ...
            <h1 className="font-coiny uppercase font-bold text-3xl md:text-4xl bg-gradient-to-br  from-brand-green to-brand-blue bg-clip-text text-transparent mt-3">
              {saleState === 1 ? 'Pre-Sale' : (saleState === 2 ? 'Public Sale' : 'Sale not started')}
            </h1>
            ...
            <div className="flex flex-col md:flex-row md:space-x-14 w-full mt-10 md:mt-14">
              ...
              <div className="flex flex-col items-center w-full px-4 mt-16 md:mt-0">
                ...
                <div className="border-t border-b py-4 mt-16 w-full">
                  <div className="w-full text-xl font-coiny flex items-center justify-between text-brand-yellow">
                    ...
                    <div className="flex items-center space-x-3">
                      <p>
                      {(saleState === 1 && whitelisted) ? '0' : new Big(totalPrice).div(new Big(10).pow(18)).toFixed(3)}{' '}
                        CRO
                      </p>{' '}
                ...
                <WalletConnectButton paused={false} isMinting={isMinting} mint={mint} />
                {/* {walletAddress ? (
                  <button
                    className={` ${
                      paused || isMinting
                ...
                <div className="mt-3">
                  ...
                  />
                  <label className='text-pink-200'>Use higher gas setting</label>
                </div>
              </div>
            </div>
            ...
            <div className="border-t border-gray-800 flex flex-col items-center mt-10 py-2 w-full">
              ...
              <div className='flex flex-row mt-4 gap-4'>
                <a href="https://discord.gg/YwPmzDCJTV"><img src='/images/discord.png' width={40} height={40} /></a>
                <a href="https://twitter.com/GhostEliteNFTs"><img src='/images/twitter.png' width={40} height={40} /></a>
              </div>
            </div>
          </div>
        </div>
